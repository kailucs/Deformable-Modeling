...
def predict_line(lineStarts,lineEnds,lineNormals,lineTorqueAxes,pcd,param,discretization,num_iter,queryDList,model,offset,p_min,p_max):
    ...
    ##Go through different points, should only be one point in the list
    # predictedForcesAll = []
    # predictedTorquesAll = []
    ## Skip force calculations, just show display in this file
    ...
    for pointNumber in num_iter:
        ...
        if OPEN3DVIS:
            ...
            for ele in surfacePtsAll:
                xyz.append(ele[0:3])
                rgb.append([0,0,1])
            open3dDisplacedPcd.points = open3d.utility.Vector3dVector(np.asarray(xyz,dtype=np.float32))
            open3dDisplacedPcd.colors = open3d.utility.Vector3dVector(np.asarray(rgb,dtype=np.float32))
        ...
        for queryD in queryDList:
            ...
            ##this will push the cylinder into the object
            penetration = 0.012 - 0.11
            #penetration = 0.012 - 0.095
            probeCenter = deepcopy(vo.sub(lineCenter,vo.mul(surfacePtsAll[i][6:9],penetration)))

            probeX = deepcopy(localXinW)
            probeY = deepcopy(localYinW)
            probeZ = vo.cross(probeX,probeY)
            transform = np.asarray(
            [[probeX[0], probeY[0], probeZ[0],  probeCenter[0]-0.015],
            [probeX[1], probeY[1], probeZ[1],  probeCenter[1]-0.045],
            ...
            if NofSurfacePts > 0:
                ...
                ###skip calculating force and torque in this file....
                ...
                colorUpperBound = 0.0236#0.012
                forceColorUpperBound = 0.004
                colorRange = colorUpperBound - colorThreshold
                greyColor = 0.38
                open3dPcd = open3d.geometry.PointCloud()
                forcePcd = open3d.geometry.PointCloud()
                xyz = []
                rgb = []
                xyzForce = []
                rgbForce = []
                maxDisp = 0
                for pt,ptNormal,original_color in zip(equilibriumPcd.points,equilibriumPcd.normals,equilibriumPcd.colors):
                    shapeVector = []
                    ...
                    ##these are for showing actual displacements, not used in the final version of the paper
                ...
                open3d.visualization.draw_geometries([open3dPcd,mesh_box_bottom])
...
def predict_circle(pcd,probedPcd,rigidPointsLocal,param,discretization,num_iter,queryDList,model,offset):
    ...
    for pointNumber in num_iter:
        ...
        for queryD in queryDList:
            ...
            ##add whole object deformation visualization            
...
rigidPointsLocal = create_circle(discretization)
...
# 2 for sloth line
#110 for lamb line
#? for vest line... forgot to record.... (1-10)
#31 for shoe line
#9 for bird
#17 for sloth cylinder
#84 for vest cylinder
#7 for show cylinder
#? for lamb cylinder..... forgot to record...
for i in [9]:
    print(i)
    if exp_N == '2' and i==285: ## pcd have holes and we end up having duplicated points
        ...
        offset = 0
        if exp_N == '4':
            predict_line(lineStarts,lineEnds,lineNormals,lineTorqueAxes,pcd_cut,param,discretization,num_iter,queryDList,model,offset,p_min,p_max)           
            #predict_circle(pcd_cut,probedPcd,rigidPointsLocal,param,discretization,num_iter,queryDList,model,offset)
        else:
            predict_line(lineStarts,lineEnds,lineNormals,lineTorqueAxes,pcd,param,discretization,num_iter,queryDList,model,offset,p_min,p_max)
            #predict_circle(pcd,probedPcd,rigidPointsLocal,param,discretization,num_iter,queryDList,model,offset)