...
%PCREGISTERICP Register two point clouds with ICP algorithm.
%   tform = PCREGISTERICP(moving, fixed) returns the rigid transformation that
...
%   PCREGISTERICP to improve accuracy and efficiency of registration.
%
...
for i = 1 : maxIterations
    ...
    if useDegree
        printer.printMessage('vision:pointcloud:checkConverge', ...
            num2str(tdiff), num2str(rdiff*180/pi), num2str(Err(i+1)));
    else
        printer.printMessage('vision:pointcloud:checkConverge', ...
...
function [metric, doExtrapolate, inlierRatio, maxIterations, tolerance, ...
    ...
    if ~(vision.internal.isRigidTransform(initialTransform))
